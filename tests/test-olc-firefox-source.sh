...
test_shared_source_script_contract() {
  ...
  [[ "$contents" == *'resolve_desktop_launch_env() {'* ]] || fail "expected the source helper to resolve the live desktop X environment for manual browser launches"
  [[ "$contents" == *'write_manual_browser_profile() {'* ]] || fail "expected the source helper to seed an isolated manual-test Firefox profile"
  [[ "$contents" == *'user_pref("security.enterprise_roots.enabled", true);'* ]] || fail "expected the source helper to trust the local CA in manual-test Firefox profiles"
  [[ "$contents" == *'open_manual_browser_window() {'* ]] || fail "expected the source helper to support a direct visible source-built browser launch"
  [[ "$contents" == *'env -u DBUS_SESSION_BUS_ADDRESS'* ]] || fail "expected the source helper to disable desktop DBus remoting for manual source-built launches"
  [[ "$contents" == *'MOZ_DBUS_REMOTE=0'* ]] || fail "expected the source helper to disable Firefox DBus remoting for manual source-built launches"
  [[ "$contents" == *'MOZ_NO_REMOTE=1'* ]] || fail "expected the source helper to disable Firefox process handoff for manual source-built launches"
  [[ "$contents" == *'--new-instance'* ]] || fail "expected the source helper to force a standalone Firefox instance for manual testing"
  [[ "$contents" == *'focus_manual_browser_window "$launcher_pid"'* ]] || fail "expected the source helper to try to activate the launched manual-test browser window"
  ...
  [[ "$contents" == *'open-window [url]  Launch the source-built Firefox in the live desktop session with an isolated test profile.'* ]] || fail "expected the source helper to expose a direct visible browser-launch command for manual testing"
...
test_docs_record_recommended_source_loop() {
  ...
  [[ "$readme_contents" == *'olc-firefox-source open-window https://localhost/'* ]] || fail "expected README to document the safe visible source-built browser launch path"
