...
FIREFOX_PATCH="${ROOT_DIR}/patches/firefox/packaged/0001-close-last-tab-to-localhost.patch"
FIREFOX_FXA_PATCH="${ROOT_DIR}/patches/firefox/packaged/0002-hide-sync-fxa-ui.patch"
...
test_vm_runs_firefox_borderless_and_maximized() {
  ...
  olc_nix="$(cat "${OLC_NIX}")"
  terminal_client="$(cat "${TERMINAL_CLIENT_SOURCE}" "${TERMINAL_CLIENT_LIFECYCLE}" "${LOCALHOST_UI_TERMINAL_OPTIONS}")"
  ...
  [[ "$contents" == *"qemu_kvm"* ]] || fail "expected VM to include QEMU/KVM for nested VM development"
  [[ "$contents" == *"virtiofsd"* ]] || fail "expected VM to include virtiofsd for nested shared repo mounts"
  [[ "$contents" == *"novnc"* ]] || fail "expected VM to include noVNC assets for browser VM screens"
  [[ "$contents" == *"xdg-utils"* ]] || fail "expected VM to include xdg-open for opening child VM screen tabs"
  [[ "$contents" == *"writeShellScriptBin \"codex\""* ]] || fail "expected VM to include a codex command wrapper"
  [[ "$contents" == *"writeShellScriptBin \"olc-launch-test-vm\""* ]] || fail "expected VM to include the nested VM launch wrapper"
  [[ "$contents" == *"OLC_DEFAULT_NOVNC_DIR"* ]] || fail "expected nested VM launch wrapper to provide pinned noVNC assets"
  [[ "$contents" == *"pkgs.novnc}/share/webapps/novnc"* ]] || fail "expected nested VM launch wrapper to use nixpkgs noVNC webapp assets"
  [[ "$contents" == *"OLC_VM_IMAGES_DIR:-/vm-images"* ]] || fail "expected nested VM launch wrapper to default to the parent-provided image mount"
  [[ "$contents" == *"OLC_VM_WORKSPACE:-/var/lib/ol-c/vms"* ]] || fail "expected nested VM launch wrapper to default to the VM workspace"
  ...
  [[ "$contents" == *"window.localStorage.setItem(draftKey(activeBuffer.path), content)"* ]] || fail "expected VM editor to keep unsaved drafts in browser local storage"
  [[ "$contents" == *"const openBuffers = new Map();"* ]] || fail "expected VM editor to keep one CodeMirror state per open file"
  [[ "$contents" == *"const themeCompartment = new Compartment();"* ]] || fail "expected VM editor to use a CodeMirror compartment for shared theme updates"
  [[ "$contents" == *"editorView.setState(buffer.state)"* ]] || fail "expected VM editor to swap CodeMirror states when switching open files"
  [[ "$contents" == *"activeBuffer.state = update.state"* ]] || fail "expected VM editor to retain each file's current CodeMirror state and undo history"
  [[ "$contents" != *"insert: content"* ]] || fail "expected VM editor not to load files by replacing text in the active undo history"
  [[ "$contents" == *"id=\"buffers\" role=\"list\""* ]] || fail "expected VM editor to show open buffers in the sidebar"
  [[ "$contents" == *"bufferStateLabel(buffer)"* && "$contents" == *"'●'"* && "$contents" == *"'◆'"* && "$contents" == *"'○'"* && "$contents" == *"'◇'"* ]] || fail "expected VM editor to show sidebar buffer edit state with unicode markers"
  [[ "$contents" == *"buffer-close"* && "$contents" == *"×"* ]] || fail "expected VM editor to expose a unicode close marker for open buffers"
  [[ "$contents" == *"window.confirm(\`Close \${buffer.name} with unsaved edits?\`)"* ]] || fail "expected VM editor to confirm before closing buffers with unsaved edits"
  [[ "$contents" == *"event.key.toLowerCase() === 's'"* && "$contents" == *"event.preventDefault();"* ]] || fail "expected VM editor to save through Ctrl+S/Cmd+S"
  [[ "$contents" != *"id=\"filebar\""* ]] || fail "expected VM editor to remove the top buffer bar"
  [[ "$contents" != *"id=\"save\""* ]] || fail "expected VM editor to avoid a visible save button"
  [[ "$contents" != *"id=\"status\""* ]] || fail "expected VM editor to remove the bottom status bar"
  ...
  [[ "$contents" == *"findTerminalColorScheme(value)"* ]] || fail "expected VM localhost UI to validate terminal color scheme choices"
  ...
  [[ "$contents" == *"browser-implementation"* ]] || fail "expected VM localhost UI to show browser implementation status"
  [[ "$contents" == *"Firefox is version "* ]] || fail "expected VM localhost UI to show the Firefox version"
  [[ "$contents" == *"OLC_FIREFOX = \"\${pkgs.firefox}/bin/firefox\";"* ]] || fail "expected VM localhost UI service to receive the packaged Firefox executable"
  [[ "$contents" == *"OLC_FIREFOX_VERSION = pkgs.firefox.version;"* ]] || fail "expected VM localhost UI service to receive the packaged Firefox version"
  ...
  [[ "$contents" != *"if (reqUrl.pathname === '/terminal/session')"* ]] || fail "expected VM stable terminal service not to keep the reload-token session creation endpoint"
  [[ "$contents" != *"sessionStorageKey: 'olc-terminal-session-token'"* ]] || fail "expected VM /terminal page not to use stored reload tokens"
  ...
  [[ "$terminal_client" == *"appConfig.wsUrl"* ]] || fail "expected VM terminal client to use the configured stable websocket URL"
  [[ "$terminal_client" != *"sessionStorage"* ]] || fail "expected VM terminal client not to store terminal reconnect tokens"
  [[ "$terminal_client" == *"const reloadWarningMessage = 'Are you sure? This terminal session will clear.';"* ]] || fail "expected VM terminal client to define the reload warning message"
  [[ "$terminal_client" == *"event.returnValue = reloadWarningMessage;"* ]] || fail "expected VM terminal client to prompt before reload"
  [[ "$terminal_client" == *"pageUnloading = true;"* ]] || fail "expected VM terminal client to distinguish reload unloads from root shell exits"
  [[ "$terminal_client" == *"if (pageUnloading)"* ]] || fail "expected VM terminal client not to close the tab during reload unload"
  ...
  [[ "$terminal_client" == *"import { Unicode11Addon } from '@xterm/addon-unicode11';"* ]] || fail "expected VM terminal client to import the xterm Unicode 11 width provider"
  [[ "$terminal_client" == *"terminal.loadAddon(unicode11Addon);"* ]] || fail "expected VM terminal client to load the Unicode 11 width provider"
  [[ "$terminal_client" == *"terminal.unicode.activeVersion = '11';"* ]] || fail "expected VM terminal client to activate Unicode 11 width rules"
  ...
  [[ "$terminal_client" == *"defaultTerminalPreferences"* ]] || fail "expected VM terminal client to import default terminal preferences"
  ...
  [[ "$terminal_client" == *"terminalThemes[terminalPreferences.colorScheme]?.[appearanceMode]"* ]] || fail "expected VM terminal client to choose the configured scheme's light or dark variant"
  ...
  [[ "$terminal_client" == *"fontFamily: selectedTerminalFontFamily()"* ]] || fail "expected VM terminal client to initialize xterm with the selected terminal font"
  [[ "$terminal_client" == *"terminal.options.fontFamily = selectedTerminalFontFamily();"* ]] || fail "expected VM terminal client to update xterm font when terminal preferences change"
  [[ "$terminal_client" == *"terminal.options.theme = theme;"* ]] || fail "expected VM terminal client to update xterm theme when terminal preferences change"
  [[ "$terminal_client" == *"fetch('/api/system/events'"* ]] || fail "expected VM terminal client to subscribe to system preference events"
  ...
  [[ "$terminal_themes" == *"tango: Object.freeze"* && "$terminal_themes" == *"background: '#2e3436'"* ]] || fail "expected VM terminal themes to include Tango light and dark variants"
  [[ "$contents" == *"fonts.packages"* && "$contents" == *"dejavu_fonts"* && "$contents" == *"inconsolata"* ]] || fail "expected VM to install configured terminal fonts"
...
test_packages_firefox_with_localhost_patch() {
  local flake_contents fast_contents patch_contents fxa_patch_contents
  flake_contents="$(cat "${FLAKE_NIX}")"
  fast_contents="$(cat "${NIX_FIREFOX_FAST}")"
  patch_contents="$(cat "${FIREFOX_PATCH}")"
  fxa_patch_contents="$(cat "${FIREFOX_FXA_PATCH}")"

  [[ "$flake_contents" == *"firefoxPackagedPatchDir = ./patches/firefox/packaged;"* ]] || fail "expected flake to define the packaged Firefox patch directory"
  [[ "$flake_contents" == *"firefoxPackagedPatchStack ="* ]] || fail "expected flake to define an ordered packaged Firefox patch stack"
  [[ "$flake_contents" == *"./patches/firefox/packaged"* ]] || fail "expected flake packaged patch stack to read from the packaged Firefox patch directory"
  [[ "$flake_contents" != *"patches/firefox/pending"* ]] || fail "expected flake packaged Firefox path not to reference pending Firefox patches"
  ...
  [[ "$flake_contents" == *"patches = (old.patches or []) ++ firefoxPackagedPatchStack;"* ]] || fail "expected flake to append the ordered packaged patch stack to firefox-unwrapped"
  ...
  [[ "$flake_contents" != *"inherit inputs"* ]] || fail "expected flake to avoid referencing an undefined inputs binding"
  [[ "$flake_contents" == *"nixosConfigurations.\"ol-c\""* ]] || fail "expected flake to expose one current ol-c NixOS configuration"
  [[ "$flake_contents" == *"modules = [ overlayModule olcModule ];"* ]] || fail "expected ol-c NixOS configuration to use the fast patched Firefox overlay"
  [[ "$flake_contents" == *"\"ol-c-image\" = self.nixosConfigurations.\"ol-c\".config.system.build.images.qemu;"* ]] || fail "expected flake to expose the upstream nixpkgs qemu qcow2 image derivation"
  ...
  [[ "$fxa_patch_contents" == *"gSecureOSFxaSyncUi"* ]] || fail "expected Sync/FxA patch to define the browser chrome UI hider"
  [[ "$fxa_patch_contents" == *"olc-fxa-sync-ui-hidden"* ]] || fail "expected Sync/FxA patch to leave a visible validation marker"
  [[ "$fxa_patch_contents" == *"#fxa-toolbar-menu-button"* ]] || fail "expected Sync/FxA patch to hide the Firefox account toolbar button"
  [[ "$fast_contents" == *"firefox_omnis=("* ]] || fail "expected fast Firefox package to inspect all runtime omni archives"
  [[ "$fast_contents" == *"firefox_patches=("* ]] || fail "expected fast Firefox package to consume an ordered Firefox patch stack"
  [[ "$fast_contents" == *"localhost_patch=\"''\${firefox_patches[0]}\""* ]] || fail "expected fast Firefox package to identify the localhost patch by stack order"
  [[ "$fast_contents" == *"fxa_sync_ui_patch=\"''\${firefox_patches[1]}\""* ]] || fail "expected fast Firefox package to identify the Sync/FxA patch by stack order"
  ...
  [[ "$fast_contents" == *"Firefox patch \$patch_file matched multiple"* ]] || fail "expected fast Firefox package to fail on ambiguous runtime asset matches"
  [[ "$fast_contents" == *"Firefox patch \$patch_file did not match any extracted"* ]] || fail "expected fast Firefox package to fail on missing runtime asset matches"
  ...
  [[ "$fast_contents" == *"openTrustedLinkIn(\"https://localhost\""* ]] || fail "expected fast Firefox package to ensure browser.js trusted tab opens use localhost"
  ...
  [[ "$fast_contents" == *"gSecureOSFxaSyncUi.init()"* ]] || fail "expected fast Firefox package to validate the Sync/FxA UI hider"
  [[ "$fast_contents" == *"olc-fxa-sync-ui-hidden"* ]] || fail "expected fast Firefox package to validate the Sync/FxA UI marker"
  [[ "$fast_contents" == *"OLC_FIREFOX_FXA_SYNC_UI_PATCH_APPLIED=1"* ]] || fail "expected fast Firefox package to leave a Sync/FxA patch marker"
