...
test_milestone2_runs_firefox_borderless_and_maximized() {
  ...
  [[ "$contents" == *"The next proof surface is <a href=\"/terminal\"><code>/terminal</code></a>"* ]] || fail "expected milestone2 localhost UI to link to the terminal proof surface"
  [[ "$contents" == *"if (reqUrl.pathname === '/terminal')"* ]] || fail "expected milestone2 localhost UI to handle /terminal"
  [[ "$contents" == *"const backendBasePath = \`/terminal/backend/"* ]] || fail "expected milestone2 /terminal page to mint a fresh backend path for each terminal page"
  [[ "$contents" == *"/terminal/assets/terminal.css"* ]] || fail "expected milestone2 /terminal page to load first-party terminal styles"
  [[ "$contents" == *"/terminal/assets/terminal.js"* ]] || fail "expected milestone2 /terminal page to load the first-party terminal client"
  [[ "$contents" == *"window.SECUREOS_TERMINAL_CONFIG"* ]] || fail "expected milestone2 /terminal page to configure a first-party terminal client"
  [[ "$contents" == *"const terminalClientJs = "* ]] || fail "expected milestone2 localhost UI to embed the terminal client asset"
  [[ "$contents" == *"const terminalClientCss = "* ]] || fail "expected milestone2 localhost UI to embed the terminal stylesheet asset"
  [[ "$contents" == *"const ttydBin = '"* ]] || fail "expected milestone2 localhost UI to use ttyd for the terminal backend"
  [[ "$contents" == *"'--uid', demoUser.uid"* ]] || fail "expected milestone2 ttyd backend to run as the demo user"
  [[ "$contents" == *"'--base-path', basePath"* ]] || fail "expected milestone2 ttyd backend to stay behind the localhost reverse proxy"
  [[ "$contents" == *"recordSocketOpen(token);"* ]] || fail "expected milestone2 terminal sessions to track active websocket clients"
  [[ "$contents" == *"recordSocketClose(token);"* ]] || fail "expected milestone2 terminal sessions to tolerate disconnects before cleanup"
  ...
  [[ "$contents" == *"services.spice-vdagentd.enable = true;"* ]] || fail "expected milestone2 to enable the SPICE guest agent daemon"
  [[ "$contents" == *"spice-vdagent"* ]] || fail "expected milestone2 to include the SPICE guest agent"
  [[ "$contents" == *"\${pkgs.spice-vdagent}/bin/spice-vdagent &"* ]] || fail "expected milestone2 xsession to start the SPICE session agent"
  ...
  [[ "$contents" != *"tmux"* ]] || fail "expected milestone2 terminal proof to avoid tmux session wrapping"
