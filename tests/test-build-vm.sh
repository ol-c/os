...
test_vm_runs_firefox_borderless_and_maximized() {
  ...
  [[ "$contents" == *"terminalClientJs: readFileSync(requireEnv('OLC_TERMINAL_CLIENT_JS'), 'utf8')"* ]] || fail "expected VM localhost UI to read the terminal client asset"
  [[ "$contents" == *"terminalClientCss: readFileSync(requireEnv('OLC_TERMINAL_CLIENT_CSS'), 'utf8')"* ]] || fail "expected VM localhost UI to read the terminal stylesheet asset"
  ...
  [[ "$contents" == *"ttydBin: requireEnv('OLC_TTYD')"* ]] || fail "expected VM localhost UI to use configured ttyd for the terminal backend"
  ...
  [[ "$contents" == *"closeUrl:"* ]] || fail "expected VM /terminal page to configure an explicit backend close URL"
  [[ "$terminal_client" == *"window.addEventListener('pagehide'"* ]] || fail "expected VM terminal client to send cleanup on pagehide"
  [[ "$terminal_client" == *"navigator.sendBeacon"* ]] || fail "expected VM terminal client to prefer sendBeacon for page close cleanup"
  [[ "$terminal_client" == *"keepalive: true"* ]] || fail "expected VM terminal client to fall back to keepalive fetch for page close cleanup"
