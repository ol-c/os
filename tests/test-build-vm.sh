...
FIREFOX_FXA_PATCH="${ROOT_DIR}/patches/firefox/0002-hide-sync-fxa-ui.patch"
...
test_packages_firefox_with_localhost_patch() {
  ...
  [[ "$flake_contents" == *"firefoxPatchStack = ["* ]] || fail "expected flake to define an ordered Firefox patch stack"
  [[ "$flake_contents" == *"./patches/firefox/0001-close-last-tab-to-localhost.patch"* ]] || fail "expected flake patch stack to include the localhost Firefox patch"
  [[ "$flake_contents" == *"./patches/firefox/0002-hide-sync-fxa-ui.patch"* ]] || fail "expected flake patch stack to include the Sync/FxA UI Firefox patch"
  ...
  [[ "$flake_contents" == *"patches = (old.patches or []) ++ firefoxPatchStack;"* ]] || fail "expected flake to append the ordered patch stack to firefox-unwrapped"
