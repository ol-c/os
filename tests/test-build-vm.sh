...
test_vm_runs_firefox_borderless_and_maximized() {
  ...
  [[ "$contents" == *"OLC_TERMINAL_UPSTREAM = \"http://127.0.0.1:9444\";"* ]] || fail "expected VM localhost UI to proxy to the stable terminal service"
  ...
  [[ "$contents" == *"reqUrl.pathname.startsWith('/terminal')"* ]] || fail "expected VM localhost UI to proxy /terminal paths"
