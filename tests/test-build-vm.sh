...
test_packages_firefox_with_localhost_patch() {
  ...
  [[ "$fast_contents" == *"prev.nodejs"* ]] || fail "expected fast Firefox package to use Node.js for structured runtime JS rewrites"
  ...
  [[ "$fast_contents" == *"Object.defineProperty(this, \"BROWSER_NEW_TAB_URL\""* ]] || fail "expected fast Firefox package to patch the global browser new-tab URL getter"
  [[ "$fast_contents" == *"return \"https://localhost\";"* ]] || fail "expected fast Firefox package to force BROWSER_NEW_TAB_URL to localhost"
  [[ "$fast_contents" == *"return AboutNewTab.newTabURL;"* ]] || fail "expected fast Firefox package to reject the stock AboutNewTab URL getter after patching"
  [[ "$fast_contents" == *"utility_overlay_path="* ]] || fail "expected fast Firefox package to identify the patched utilityOverlay.js runtime asset"
  [[ "$fast_contents" == *"patched Firefox browser commands runtime asset still contains stock new-tab URL references"* ]] || fail "expected fast Firefox package to fail if browser commands can still use Firefox's stock new-tab URL"
  ...
  [[ "$fast_contents" == *"patched Firefox browser.js runtime asset still contains stock new-tab URL references"* ]] || fail "expected fast Firefox package to fail if browser.js can still use Firefox's stock new-tab URL"
