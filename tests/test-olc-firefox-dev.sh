...
test_patched_firefox_command_contract() {
  ...
  [[ "$contents" == *"printf 'packaged_patches=%s\\n' \"''\${packaged_patches[*]}\""* ]] || fail "expected patched-firefox to record packaged patches in runtime order"
  [[ "$contents" == *"printf 'pending_patches=%s\\n' \"''\${pending_patches[*]}\""* ]] || fail "expected patched-firefox to record pending patches in runtime order"
