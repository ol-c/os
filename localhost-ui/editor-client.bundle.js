(() => {
  ...
  var HeightMap = class _HeightMap {
    ...
    static of(nodes2) {
      if (nodes2.length == 1)
        return nodes2[0];
      let i = 0, j = nodes2.length, before = 0, after = 0;
      for (; ; ) {
        if (i == j) {
          if (before > after * 2) {
            let split = nodes2[i - 1];
            if (split.break)
              nodes2.splice(--i, 1, split.left, null, split.right);
            else
              nodes2.splice(--i, 1, split.left, split.right);
            j += 1 + split.break;
            before -= split.size;
          } else if (after > before * 2) {
            let split = nodes2[j];
            if (split.break)
              nodes2.splice(j, 1, split.left, null, split.right);
            else
              nodes2.splice(j, 1, split.left, split.right);
            j += 2 + split.break;
            after -= split.size;
          } else {
            break;
      ...
      if (nodes2[i - 1] == null) {
        brk = 1;
        i--;
      } else if (nodes2[i] == null) {
        brk = 1;
        j++;
      }
      return new HeightMapBranch(_HeightMap.of(nodes2.slice(0, i)), brk, _HeightMap.of(nodes2.slice(j)));
    }
  };
  ...
  var HeightMapText = class _HeightMapText extends HeightMapBlock {
    ...
    replace(_from, _to, nodes2) {
      let node = nodes2[0];
      if (nodes2.length == 1 && (node instanceof _HeightMapText || node instanceof HeightMapGap && node.flags & 4) && Math.abs(this.length - node.length) < 10) {
  ...
  var HeightMapGap = class _HeightMapGap extends HeightMap {
    ...
    replace(from, to, nodes2) {
      let after = this.length - to;
      if (after > 0) {
        let last = nodes2[nodes2.length - 1];
        if (last instanceof _HeightMapGap)
          nodes2[nodes2.length - 1] = new _HeightMapGap(last.length + after);
        else
          nodes2.push(null, new _HeightMapGap(after - 1));
      }
      if (from > 0) {
        let first = nodes2[0];
        if (first instanceof _HeightMapGap)
          nodes2[0] = new _HeightMapGap(from + first.length);
        else
          nodes2.unshift(new _HeightMapGap(from - 1), null);
      }
      return HeightMap.of(nodes2);
    }
    ...
    updateHeight(oracle, offset = 0, force = false, measured) {
      ...
      if (measured && measured.from <= offset + this.length && measured.more) {
        let nodes2 = [], pos = Math.max(offset, measured.from), singleHeight = -1;
        if (measured.from > offset)
          nodes2.push(new _HeightMapGap(measured.from - offset - 1).updateHeight(oracle, offset));
        ...
        if (pos <= end)
          nodes2.push(null, new _HeightMapGap(end - pos).updateHeight(oracle, pos));
  ...
  function buildTree(data) {
    ...
    function takeFlatNode(parentStart, minPos, children2, positions2) {
      ...
      if (nodeCount) {
        ...
        for (let i = nodes2.length - 3, j = 0; i >= 0; i -= 3) {
          buffer2[j++] = nodes2[i];
          buffer2[j++] = nodes2[i + 1] - start;
          buffer2[j++] = nodes2[i + 2] - start;
          buffer2[j++] = j;
        }
        children2.push(new TreeBuffer(buffer2, nodes2[2] - start, nodeSet));
        positions2.push(start - parentStart);
      }
    }
  ...
  var activePath = null;
  var openBuffers = /* @__PURE__ */ new Map();
  var entries = [];
  var editorPreferences = { ...defaultTerminalPreferences };
  ...
  function selectedTheme() {
    return terminalThemes[editorPreferences.colorScheme]?.[appearanceMode] ?? terminalThemes[defaultTerminalPreferences.colorScheme][appearanceMode];
  }
  function selectedFontFamily() {
    return findTerminalFont(editorPreferences.font)?.cssFamily ?? findTerminalFont(defaultTerminalPreferences.font).cssFamily;
  }
  function themeExtension(theme2) {
    return [
      EditorView.theme({
        "&": {
          height: "100%",
        ...
        ".cm-cursor": {
          borderLeftColor: theme2.accent
        },
        ".cm-selectionBackground, ::selection": {
          backgroundColor: `${theme2.selection} !important`
        },
        ".cm-gutters": {
          backgroundColor: theme2.panel,
          color: theme2.muted,
          borderRightColor: theme2.line
        },
      ...
      syntaxHighlighting(HighlightStyle.define([
        { tag: tags.keyword, color: theme2.keyword },
        { tag: tags.string, color: theme2.string },
        { tag: tags.number, color: theme2.number },
        { tag: tags.comment, color: theme2.comment },
        { tag: tags.variableName, color: theme2.variable },
        { tag: tags.invalid, color: theme2.error }
      ]))
    ];
  }
  ...
  function createBuffer({ path, name: name2, content: content2, mtimeMs }) {
  ...
  function renderBuffers() {
    ...
    nodes.buffers.replaceChildren(...buffers.map((buffer) => {
      ...
      row.innerHTML = '<span class="buffer-name"></span><span class="buffer-state"></span><button class="buffer-close" type="button" title="Close">\xD7</button>';
      row.querySelector(".buffer-name").textContent = buffer.name;
      row.querySelector(".buffer-state").title = buffer.path === activePath ? bufferDirty(buffer) ? "selected with unsaved edits" : "selected" : bufferDirty(buffer) ? "open with unsaved edits" : "open";
  ...
  function persistDraft() {
    ...
    const content2 = currentContent();
    if (content2 === activeBuffer.savedContent) {
      window.localStorage.removeItem(draftKey(activeBuffer.path));
      return;
    }
    window.localStorage.setItem(draftKey(activeBuffer.path), content2);
  }
  function applyTheme() {
    const theme2 = selectedTheme();
    document.documentElement.style.setProperty("--editor-bg", theme2.background);
    document.documentElement.style.setProperty("--editor-fg", theme2.foreground);
    document.documentElement.style.setProperty("--editor-muted", theme2.muted);
    document.documentElement.style.setProperty("--editor-line", theme2.line);
    document.documentElement.style.setProperty("--editor-accent", theme2.accent);
    document.documentElement.style.setProperty("--editor-panel", theme2.panel);
    document.documentElement.style.setProperty("--editor-panel-fg", theme2.panelForeground);
    document.documentElement.style.setProperty("--editor-error", theme2.error);
  ...
  function renderTree() {
    ...
    nodes.tree.replaceChildren(...visible.map((entry) => {
      ...
      treeState.title = buffer ? buffer.path === activePath ? bufferDirty(buffer) ? "selected with unsaved edits" : "selected" : bufferDirty(buffer) ? "open with unsaved edits" : "open" : "";
